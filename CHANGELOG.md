...
- **正文显示根治（渐进增强）**：三套模板正文默认可见，只有当独立零依赖的揭示脚本成功注册 IntersectionObserver 后才进入「隐藏 → 滚动揭示」动画模式（`html.dw-anim`）；主脚本与揭示脚本分离、Lucide/localStorage 全部守卫——脚本禁用、CDN 全挂或沙箱禁脚本时正文必显示；validate.py 新增渐进增强断言。
- **正文显示根治（IO 兜底）**：渲染取证发现 IntersectionObserver 回调在部分环境不触发，导致 `.visible` 始终为 0、正文停于隐藏态。揭示脚本改为三层保障：初始几何检测立即揭示视口内内容、滚动/resize 几何兜底、1.5s 强制全量显示；reduced-motion 下补 `clip-path:inset(0)!important`。
- **动画恢复**：强制显示改为「1.5s 揭示视口内 + 8s 兜底全量」，滚动揭示动画不再被提前跳过；`prefers-reduced-motion` 从「全禁动画」改为「只禁高频/循环/位移动画」，淡入与内容揭示保留，正文与动效兼得。
- **滚动动画根治（IO 唯一驱动）**：定位「正文可见但无动效」根因——揭示脚本的 1.5s 视口强制显示与 8s 全量兜底会抢在滚动前把所有章节标记为可见，进入视口时过渡已无从播放；scroll/resize 几何兜底也会抢在 IntersectionObserver 前打标记。三套模板改为 IO 唯一揭示源：仅当 2.5 秒后全文仍无任何章节被揭示（IO 完全失效或首屏恰好无章节）时才一次性全量显示，其余情况滚动揭示动画全程保留；删除和纸模板主脚本中重复的第二套 IO 逻辑；`prefers-reduced-motion` 仅禁高运动（扫描线/glitch/打字机/跑马灯/视差位移），正文淡入与滚动揭示保留；validate.py 新增防回归断言（零揭示兜底、无抢跑兜底、无重复揭示逻辑、RM 不强制正文全显）。
