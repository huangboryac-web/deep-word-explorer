...
| 比较两个概念/地点 | `words=[词A, 词B]`，生成对比页（总览表 + 差异分析） |
...
从 8 套里选一套——**不允许自定义 hex 值**，保护美学比给自由更重要。切换主题只需替换 `template-article.html` 开头 `:root{}` 里的变量，或在 `body` class 上指定 `theme-*`。
...
| 🟢 **终端绿** | `#0f2418` / `#eef7ef` | 科技、编程、极客文化 |
| 🧧 **朱印和纸** | `#2b2118` / `#faf5ea` | 东方文化、东方人物与机构、传统 |
| 🎨 **孟菲斯波普** | `#26232a` / `#fff6ec` | 艺术设计、热词、青年文化 |

主题推荐规则：哲学/人文/通用 → 墨水经典；科技/AI/数学 → 靛蓝瓷；自然/地理/生态 → 森林墨；历史/文学/书籍 → 牛皮纸；艺术/设计/建筑 → 沙丘；科技/编程/极客文化 → 终端绿；东方文化/东方人物与机构 → 朱印和纸；艺术设计/热词/青年文化 → 孟菲斯波普。
...
**为什么不允许自定义颜色？**
重点是稳定产出。自由选色很容易破坏整体「电子墨水」风格，所以只允许从 8 套预设里选。
...
**能一次解析多个词吗？**
可以。`words` 支持 2–8 个词并行解析（并发上限 3），每个词产出独立页面，并自动生成 `index.html` 对比页（总览表 / 并排时间线 / 交叉引用 / 关键差异）。
